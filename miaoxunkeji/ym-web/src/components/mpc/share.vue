	...
	export default{
		...
		methods:{
			changqr(){
				...
				var test = dz.substring(0,dz.indexOf("new")) + 'phone/detail/miao_mpc.html?id=' + mid;
		...
		mounted(){
console.log('2.加载组件share，数据从about和home拿')
var that=this
this.changqr();
this.changer(false);
this.index=this.$route.params.page
 ...
 //      that.mpcxq=data.results
          ...
          if(this.mpcxq.group.type=='group'){
            console.log('group')
            that.location="group"
          }else if(this.mpcxq.group.type=='company'){
            console.log('company')
             that.location="company"
          }else if(this.mpcxq.group.type=='private'){
            console.log('private')
