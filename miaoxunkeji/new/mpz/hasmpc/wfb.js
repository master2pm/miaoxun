     ...
     data:function(){
      return {
        ...
        what:false,//底部的低栏
           ...
           datas:this.$store.state.wfb// 未发布
      }
     },
      computed:{
      watch(){
        this.datas=this.$store.state.wfb
      }
    },
    methods:{
            // 更新苗木状态确定按钮
      ...
      gotjxmm(objectId,item){
         this.$store.commit("whereTjxmm",item)
          this.$router.push({'name':'adds1',params:{'name':this.$route.params.name,'objectId':this.$route.params.objectId,'biaoti':4,'tjId':objectId,'status':item.status}})
      },
      // 更新苗木状态按钮
      updataStatus(){
            // 111如果不是删除按钮的时候执行更新状态操作
          ...
          if(this.what==true){
            return
                   ...
                   if(item.mu_type!=''&&item.mu_type=="袋苗"){
                        if(item.mu_package_weight!=''){
                             iss=true
                        }else{
                             iss=false
                        }
                   }else if(item.mu_type!=''&&item.mu_type!="袋苗"){
                        if(item.mu_dj_max==''&&item.mu_dj_min==''&&item.mu_mj_max==''&&item.mu_mj_min==''){
                             iss=false
                        }else{
                             iss=true
                        }
                   }
                   if(iss==true&&item.mu_name!=''&&item.mu_type!=''&&(item.mu_gf_max!=''||item.mu_gf_min!='')&&(item.mu_zg_max!=''||item.mu_zg_min!='')&&item.mu_price!=''&&item.mu_stock!=''&&item.mu_photo.length>0){
                          // 可以继续的
                        this.alerts=false
                   }else{
                        // 返回
