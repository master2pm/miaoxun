var fangdatu = {
	...
	<span>{{datas.mu_price=='-1'?'面议':(datas.mu_type=='草坪'?('￥'+datas.mu_price+'/㎡'):('￥'+datas.mu_price+'元/株'))}}</span>
	...
	<!-- 1非草坪 -->
	<ul class="rf f12 c4A494B" v-if="datas.mu_type!='草坪'">
...
<!-- 2草坪 -->
					...
					<ul class="rf f12 c4A494B" v-if="datas.mu_type=='草坪'">
						...
						<li v-if="datas.lawn_length!=undefined&&datas.lawn_length!=''">
								草坪长：{{datas.lawn_length}}cm
						</li>
						<li v-if="datas.lawn_width!=undefined&&datas.lawn_width!=''">
								草坪宽：{{datas.lawn_width}}cm
						</li>
						<li v-if="datas.lawn_density!=undefined&&datas.lawn_density!=''">
								密度：{{datas.lawn_density}}%
						</li>
						<li v-if="datas.lawn_amount!=undefined&&datas.lawn_amount!=''">
								每捆数量：{{datas.lawn_amount}}片
						</li>						
					</ul>
...
<!-- onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}" onafterpaste="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}" -->
	...
	{{datas.mu_type=='草坪'?'㎡':'株'}}（库存{{datas.mu_stock}}{{datas.mu_type=='草坪'?'㎡':'株'}}）
	</div>
	<!-- 加入购物车按钮 -->
	<div class="cart" >
	...
	<div class="f12 c60B346 relative" style="">{{item.mu_price=='-1'?'面议':(item.mu_type=='草坪'?('￥'+item.mu_price+'/㎡'):('￥'+item.mu_price+'元/株'))}}</div>
	</div>
		...
		methods:{
           ...
           if(this.shuliang.length==1){
			...
			changqr(str){
				...
				var tou = dz.substring(0,dz.indexOf("new"));
				var temp = str.substring(str.length-1);
				var test = '';
				console.log(tou)
				if(temp == 1){
					test = tou + 'phone/detail/miao_mm.html?mid=' + that.datas.mu_ground.objectId  + "&cid=" + that.datas.objectId;
					console.log(test);
				}else{
					test = tou + 'phone/detail/miao_mpc.html?id=' + that.datas.mu_ground.objectId;
					console.log(test);
				}
  ...
  this.bigbig=true
			...
			storeShows(e,n,objectId,type,amount){
				if(this.storeShow==false){
					this.storeShow=true
						if(common_fangdatuIndex!=-1){
						        if(this.$parent.Yulans!=undefined){
						        	// 预览的列表的放大图
						        	this.$parent.datas[common_fangdatuIndex].isfavor=1
						        	  console.log('Yulans1')
						        }
					    }
					...
					if(common_fangdatuIndex!=-1){
						        if(this.$parent.Yulans!=undefined){
						        	// 预览的列表的放大图
						        	this.$parent.datas[common_fangdatuIndex].isfavor=0
						        	  console.log('Yulans0')
						        }
					    }
					common_adds(e,n,objectId,type,amount)
			...
			// 鼠标点击放大图的外面的时候让弹出框消失
			falses(e){
				// 防止放大图和头部不是同级的时候导致无法比较层叠性，打开的时候让头部层叠性降低，关闭终极放大图的时候让头部层叠性恢复
	            $("header").removeClass('header10')
				common_fangdatuIndex=-1
	...
	$("header").addClass('header10')
			...
			$("#fx1").hide();
							...
							$.get(Boss+"/tree?where="+encodeURI(JSON.stringify(where))
								,function(data){
									if(data.status==1){
