	...
	<div class="wai_kuangs"  style="z-index:2006;">
		...
		<div  v-if="finalArrs.length!=0" id="myCarousels" class=" carousel zhongji " data-ride="carousel" data-interval=false style="height:100%;width:100%;" >
               ...
               <ol class="carousel-indicators">
						<li data-target="#myCarousels" :data-slide-to="index" :class="{active:index==0}" v-for="(item,index) in finalArrs"></li>
			   </ol> 
			...
			<div id="myCarousels1"  class="carousel-inner" style="height:100%;width:100%;">
				<div class="item relative" :class="{active:index==0}" id="getheight" v-for="(item,index) in finalArrs" style="height:100%;width:100%;">
					<img :src="item" alt="slide" style="max-width:none;" class="jz cursor"
					@mouseenter="drag_over($event)" @mouseleave="drag_out($event)">
					<!-- <div class="carousel-caption">标题 {{index}}</div> -->
				</div>

	  <a :style="{display:this.$route.name=='sfz'||this.$route.name=='sfz2'||this.$route.name=='touxiang'||this.$route.name=='message'?'none':'block'}" class="carousel-control left tingzhi" 
      data-slide="prev"  @click="toPrevs()" id="lbp">
      <img src="/static/tu-detail/1-69 (2).png"  id="home_img1" class="czjz cursor" style="left:30px;">
    </a>
    <a :style="{display:this.$route.name=='sfz'||this.$route.name=='sfz2'||this.$route.name=='touxiang'||this.$route.name=='message'?'none':'block'}" class="carousel-control right tingzhi" 
    data-slide="next"  @click="toNexts()"  id="lbn">
    <img src="/static/tu-detail/1-70 (2).png"  id="home_img2" class="czjz cursor" style="right:30px;">
</a>
...
<div class="f26  spjz center" style="color:white;padding:0 250px;width:524px;height:75px;bottom:0;">
    <!--1/5-->
    <span class="lf" style="dispaly:none;margin-top:22px;display:none;">
   ...
   <!-- 加 -->
	<img @click="final3s($event)"  src="/static/tu-detail/1-72 (2).png"  class="final3 ff czjz cursor" style="display:none;z-index:10;bottom:10px;left:450px;">
	<!-- 减 -->
	<img @click="final4s($event)"  src="/static/tu-detail/1-73 (2).png"  class="final4 ff czjz cursor" style="display:none;z-index:10;bottom:10px;left:500px;">

	

    <!--已设为封面  设为封面-->
    <span class="rf" style="display:none;margin-top:22px;margin-left:20px;">
         已设为封面
    </span>

    <!--删除-->
    <span @click="removePhoto()" class="rf cursor" style="display:none;margin-top:22px;">
         删除
    </span>
</div>
  ...
  toNexts(e){
      ...
      initWheel(){
		...
		mounted(){
			...
			this.toNum(this.number)
			console.log("fangdazhongji:"+this.number)
